...
- [x] 1.9 Перевірити `turbo dev` — одночасний запуск site (3000) та api (4000)
...
- [x] 2.7 Написати та застосувати першу міграцію (`prisma migrate dev`)
...
- [x] 7.5 Розробити сторінку форми запису `/book` з валідацією (react-hook-form + Zod)
- [x] 7.6 Розробити сторінку входу `/login` (введення телефону → OTP → Гараж)
- [x] 7.7 Розробити сторінку Гаражу `/garage` — список авто (server component + suspense)
- [x] 7.8 Розробити сторінку деталей авто `/garage/[vehicleId]` — дані + сервісна книга
- [x] 7.9 Реалізувати форму доповнення профілю (перший вхід shadow-користувача)
...
- [x] 8.4 Перевірити повний флоу: гостьовий запис → SMS → вхід через OTP → Гараж

## 9. Фінальна перевірка та документація

- [x] 9.1 Перевірити golden path: гість → форма запису → SMS → OTP вхід → Гараж з авто
- [x] 9.2 Перевірити edge case: повторний запис того самого телефону
- [x] 9.3 Перевірити edge case: порожній Гараж (нові акаунти)
- [x] 9.4 Оновити `obsidian/00_index.md` — Фаза 1 в статусі `🟢 В розробці`
- [x] 9.5 Зафіксувати початковий commit монорепо
